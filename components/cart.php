...
$query = mysqli_query($link, "SELECT item_id, name, cost, count, extension  FROM cart INNER JOIN items ON cart.item_id = items.id WHERE client = '$active_user'");
    ...
    $main_content .= "
        <div class=\"cart-item\" id=\"$item[item_id]\">
            ...
            <div class=\"cart-item-info\"><h3 class=\"goods-item-heading\">$item[name]</h3>
                <p class=\"goods-item-text cart-item-price\">Цена: $total_item_cost рублей.</p>
                <input type=\"number\" min=\"1\" max=\"99\" class=\"cart-item-quantity\" value=\"$item[count]\" id=\"input-$item[item_id]\" readonly>
